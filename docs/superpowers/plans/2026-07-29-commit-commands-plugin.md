...
foreach ($required in @('미병합', 'current branch', 'dirty', 'locked', '승인', 'Drift', '첫 실패', 'git worktree remove -- <safely-quoted-path>', 'git branch -d -- <safely-quoted-validated-branch>', 'active shell의 개별 argv', 'command 문자열로 조합하거나 eval하지 않는다.', '각 worktree 제거 직전', '각 branch 삭제 직전', 'mapped worktree branch ref·HEAD', 'remaining mutation')) {
