...
> 게스트 OS 설정(netplan/hostname/disk/swap/sysctl/limits/machine-id)은 **하단 Guest OS 설정 섹션**에서 **3계층 접근**(qm guest exec / MCP execute_vm_command / HTTP API agent)으로 자동화.
...
VM 부팅 후 게스트 내부 OS 설정. **agent=1 선행 필수**(config 단계 `--agent 1` — virtio-serial channel 생성). 3계층은 실행 환경·한계가 상이 → 상황별 선택.
...
| **3. HTTP API agent** | `POST /nodes/{node}/qemu/{id}/agent` | 외부(HTTP) | O (`arg[]=["/bin/bash","-c","..."]`) | 자동화 최종형. 복잡 스크립트·쓰기 |

**선택 기준**: 노드 쉘 있으면 **1(qm)** → 외부 자동화는 **2(MCP, 단일/읽기)** 또는 **3(HTTP, 복잡/쓰기)**. MCP는 shell 미지원이므로 복잡 스크립트·쓰기는 HTTP API(3) 사용.
...
# FAIL (shell 구문) → HTTP API(3) 또는 qm(1) 사용
execute_vm_command(command="lsblk && df -h")                # && 미지원
execute_vm_command(command="cat /etc/hosts | grep worker")  # | 미지원
```

> 복잡 스크립트·쓰기(set)는 qm guest exec(노드 쉘) 또는 HTTP API agent(`arg[]=["/bin/bash","-c","..."]`). 기존 운영 장비(105/106/107/111)는 **읽기만**, 신규(116)는 쓰기 허용.

### cross-node (pvesh agent)
`qm guest exec`는 **소속 노드 쉘**에서만. cross-node 자동화는 HTTP `/agent` endpoint (상단 HTTP API 매핑).

## HTTP REST API 매핑 (skill 최종 형태)

pvesh/qm은 Proxmox 노드 쉘 의존(CLI 래퍼). **skill 최종은 HTTP REST API** — 외부(Mac/CI/MCP)에서 API token 인증으로 호출, 의존성 無. pvesh로 패턴 검증 후 HTTP endpoint로 일반화.

| pvesh / qm | HTTP endpoint | Method |
| :--- | :--- | :--- |
| `pvesh create .../clone` | `/api2/json/nodes/{node}/qemu/{src}/clone` | POST |
| `pvesh set .../config` | `/api2/json/nodes/{node}/qemu/{id}/config` | PUT |
| `pvesh set .../resize` | `/api2/json/nodes/{node}/qemu/{id}/resize` | PUT |
| `qm guest exec` | `/api2/json/nodes/{node}/qemu/{id}/agent` | POST (`command=["/bin/bash","-c","..."]`, shell 지원) |
| `pvesh get .../config` | `/api2/json/nodes/{node}/qemu/{id}/config` | GET |

**인증**: Proxmox API Token (Token ID `user@realm!tokenid` + Secret), `Authorization: PVEAPIToken=...` header. 비인터랙티브 자동화.

**agent endpoint 복잡 스크립트**: MCP execute_vm_command(shell 미지원)와 달리, HTTP agent endpoint는 `command`를 **배열(arg[])**로 전달 → `/bin/bash -c`로 감싸면 shell 스크립트 가능 (qm guest exec와 동일 표현력). command 인자를 URL-encoded form 으로 반복 전달:

```bash
curl -k -H "Authorization: PVEAPIToken=..." \
  -d 'command=/bin/bash' -d 'command=-c' \
  -d 'command=growpart /dev/sda 2 && resize2fs /dev/sda2' \
  https://{node}:8006/api2/json/nodes/{node}/qemu/{id}/agent
```

> MCP `execute_vm_command` = 이 HTTP agent endpoint 래퍼지만 **command를 단일 문자열로만 전달** → shell 미지원. HTTP 직접 호출은 배열(arg[]) 전달 가능 → 복잡 스크립트 지원.

**왜 HTTP API**: pvesh/qm은 Proxmox 노드 쉘 필요 → 범용 자동화 한계. HTTP API는 외부 호출 가능(CI/MCP/다른 시스템 연동). pvesh로 시퀀스 검증 → 동일 패턴을 HTTP endpoint로 일반화.
...
| **MCP execute_vm_command shell 미지원** | 단일 명령만. `&&`/`|`/`>` FAIL → 읽기 위주 사용. 복잡 스크립트·쓰기는 qm guest exec 또는 HTTP API `arg[]=/bin/bash -c` |
...
> config에 `--cpu host --numa 1 --agent 1` 포함(공식 필수). 게스트 OS 설정은 상단 Guest OS 섹션 3계층 접근.
...
- **게스트 접근 3계층**: qm guest exec(기본) / MCP execute_vm_command(외부, 읽기, 단일 명령) / HTTP API agent(자동화, 복잡 스크립트 arg[])
