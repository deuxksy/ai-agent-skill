---
name: setup
description: "dotfiles 기반 호스트 설정 종합. hostname 감지해 패키지 설치(/infra:packages 위임), stow 배포, sops 복호화, Tailscale Aperture 설정 실행. ~/git/dotfiles 존재 필수. Use when 신규 머신 설정이나 재구성 시."
---
...
| :--- | :--- | :--- | :--- |
| axiom | macOS | `base`, `axiom` | packages, stow, sops, Aperture |
| eve | macOS | `base`, `eve` | packages, stow, sops, Aperture |
| mo | NixOS | `base`, `mo` | packages (nixos-rebuild), sops, Aperture |
| girl | SteamOS | `base`, `girl` | packages (mise) |
| walle | Proxmox (Debian) | `base`, `walle` | packages (apt), sops |
...
- **패키지 설치 → stow 순서**: brew/nix/mise가 stow를 설치하므로 패키지 설치 먼저. 패키지 설치는 `/infra:packages`로 위임
