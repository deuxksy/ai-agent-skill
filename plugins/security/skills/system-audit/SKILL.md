---
name: system-audit
description: "보안 취약점 감사. CVE → NVD/NIST → CVSS → CISA KEV → upgrade 흐름. macOS, Linux, 언어별 패키지 매니저 크로스 플랫폼 지원. 패키지 업그레이드는 /infra:packages 사용."
---
...
- **upgrade는 별도 스킬**: 취약점 안내만, 실제 업그레이드는 `/infra:packages`
