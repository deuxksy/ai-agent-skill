---
name: code-audit
description: "정적 코드 보안 분석. SAST scan → CWE 분류 → OWASP Top 10 매핑 → 수정 제안. 기본값은 현재 프로젝트 디렉토리. 다른 대상 스캔 시 경로 지정. 패키지 수준 CVE 점검은 /security:system-audit 사용."
---
...
```text
/security:code-audit           → 현재 프로젝트 스캔
/security:code-audit <path>    → 특정 경로 스캔
```
...
- **재스캔**: 수정 후 `/security:code-audit` 재실행하여 회귀 확인
- **패키지 취약점**: `/security:system-audit`으로 의존성 CVE 확인
...
- **A06(Vulnerable Components)은 제외**: 의존성 취약점은 `/security:system-audit` 영역
