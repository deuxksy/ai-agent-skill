---
name: knee
description: "점진적 VU 상승 래더로 시나리오별 knee(포화 변곡점)와 MAX 처리량 Req/s를 탐색. 판정룰(이상/평탄/진행 우선순위 테이블)과 런 간 드레인 게이트를 내장해 자동 종료. 'knee 찾기', 'MAX TPS', '래더 테스트'에서 사용."
---

# Knee — 점진 VU 탐색

> **지표 용어 (혼용 금지)**: 판정·기록의 처리량은 전부 **Req/s (req/s)** — 개별 HTTP 요청 기준 (Transaction parent 제외). **TPS (tx/s)**는 업무 루프(Transaction) 완주 수/초로, `TPS = Req/s ÷ 루프당 HTTP 수(JMX 스텝 수)` 환산값만을 그 라벨로 쓴다. 상세 정의는 run 스킬 '지표 정의' 참조.
