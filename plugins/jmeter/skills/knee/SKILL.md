...
- **평탄 후 knee 보간 우선 (2026-08-29 3-4 도그푸드)**: 판정 4 평탄을 만나면 상방 확인보다 **MAX 후보 피크와 직전 포인트의 중간 VU 1점**을 먼저 실행한다 — 피크가 두 포인트 사이에 숨어 있을 수 있다 (실측: 직전 포인트가 피크로 보였으나 중간 보간에서 실제 피크가 그보다 높았음). 보간 결과가 두 인접 점과 ±5% 이내면 플래토(평평한 정점)로 확정하고 종료한다.
