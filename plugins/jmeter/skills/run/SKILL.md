...
폴더: `results/{시나리오명의 하이픈→언더스코어}/$(date +%y%m%d-%H%M%S)-T{T}x{n}_R{ramp}_D{d}-RP{rp}` (부하원 타임존) — 시나리오 하위 디렉토리에 타임스탬프 우선 폴더 생성. 정렬 시 실행 시각순으로 쌓여 시간 흐름 탐색에 유리, 시나리오 prefix는 부모 디렉토리와 중복이라 생략 (2026-09-04 변경, 소급 적용 완료 — 이전 규칙 `{시나리오}-T…-RP…-{timestamp}`는 2026-08-30~09-04 사용)
