...
폴더: `results/{시나리오명의 하이픈→언더스코어}/{시나리오명의 하이픈→언더스코어}-T{T}x{n}_R{ramp}_D{d}-RP{rp}-$(date +%y%m%d-%H%M%S)` (부하원 타임존) — 시나리오 하위 디렉토리에 RP 태그 폴더 생성. 정렬 시 시나리오→프로파일→R1/R2/R3 순으로 쌓여 세로 비교에 유리 (2026-08-30 확정)
