...
이전 세션의 handoff를 읽어 작업을 복원한다. 목표: `/sessions:handoff`로 저장한 진행상태를 new session에서 안전하게 복원.
...
**빈 세션 처리**: handoff의 `## 작업 목표`가 `No active work.` 이면 복원할 task가 없다. "이 handoff는 빈 세션(No active work) — 복원할 작업 없음." 출력 후 종료 (§3~§6 건너뜀).
